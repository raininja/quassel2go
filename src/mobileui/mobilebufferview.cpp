...
void MobileBufferView::init() {
  ...
  // mobile quassel drag/drop is too unintuitive. merging must be done differently
...
void MobileBufferView::setModel(QAbstractItemModel *model) {
...
void MobileBufferView::setFilteredModel(QAbstractItemModel *model_, BufferViewConfig *config) {
...
void MobileBufferView::setSelectionModel(QItemSelectionModel *selectionModel) {
...
void MobileBufferView::setConfig(BufferViewConfig *config) {
  if(_config == config)
    return;
...
void MobileBufferView::setRootIndexForNetworkId(const NetworkId &networkId) {
...
void MobileBufferView::joinChannel(const QModelIndex &index) {
  ...
  //  qDebug() << "MobileBufferView::joinChannel" << index;

  Client::userInput(bufferInfo, QString("/JOIN %1").arg(bufferInfo.bufferName()));
}

void MobileBufferView::keyPressEvent(QKeyEvent *event) {
...
void MobileBufferView::dropEvent(QDropEvent *event) {
...
void MobileBufferView::removeSelectedBuffers(bool permanently) {
  if(!config())
    return;
...
void MobileBufferView::rowsInserted(const QModelIndex &parent, int start, int end) {
...
void MobileBufferView::on_layoutChanged() {
...
void MobileBufferView::on_configChanged() {
  Q_ASSERT(model());
...
void MobileBufferView::storeExpandedState(const QModelIndex &networkIdx) {
...
void MobileBufferView::setExpandedState(const QModelIndex &networkIdx) {
...
void MobileBufferView::dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight) {
...
void MobileBufferView::toggleHeader(bool checked) {
  QAction *action = qobject_cast<QAction *>(sender());
  header()->setSectionHidden((action->property("column")).toInt(), !checked);
}

void MobileBufferView::contextMenuEvent(QContextMenuEvent *event) {
  QModelIndex index = indexAt(event->pos());
  if(!index.isValid())
    index = rootIndex();
...
void MobileBufferView::addActionsToMenu(QMenu *contextMenu, const QModelIndex &index) {
...
void MobileBufferView::addFilterActions(QMenu *contextMenu, const QModelIndex &index) {
  BufferViewFilter *filter = qobject_cast<BufferViewFilter *>(model());
  if(filter) {
...
void MobileBufferView::menuActionTriggered(QAction *result) {
...
void MobileBufferView::nextBuffer() {
  changeBuffer(Forward);
}

void MobileBufferView::previousBuffer() {
  changeBuffer(Backward);
}

void MobileBufferView::changeBuffer(Direction direction) {
...
void MobileBufferView::wheelEvent(QWheelEvent* event) {
...
QSize MobileBufferView::sizeHint() const {
...
{
  if(_tapSuppressor->handleMouseEvent(event))
    return true;

  return QTreeView::viewportEvent(event);
}

void MobileBufferView::paintEvent ( QPaintEvent * event )
{
  QTreeView::paintEvent(event);
  _tapSuppressor->scroller()->paintEvent(event);
}

bool MobileBufferView::event( QEvent * event )
{
...
MobileBufferViewDelegate::MobileBufferViewDelegate(QObject *parent)
  : QStyledItemDelegate(parent)
{
}

void MobileBufferViewDelegate::customEvent(QEvent *event) {
...
bool MobileBufferViewDelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index) {
